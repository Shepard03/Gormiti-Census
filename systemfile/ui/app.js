...
// Tiene sincronizzato il video a schermo intero (se aperto) con il
// personaggio attualmente mostrato, esattamente come updateCardView() fa
// gia' per la carta a schermo intero. Se non c'e' nessun video per il nuovo
// personaggio mentre il fullscreen e' aperto, lo chiude automaticamente.
function updateFullscreenVideoView(videos) {
    const fsOverlay = document.getElementById('fullscreen-overlay');
    const isOpen = fsOverlay.style.display === 'flex';
    if (!isOpen) return; // niente da sincronizzare, si aggiornerà da sé alla prossima apertura
...
function renderDetail() {
  ...
  updateFullscreenVideoView(videos);
